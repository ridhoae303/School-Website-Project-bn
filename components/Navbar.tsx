      ...
      <nav className="sticky top-0 z-40 bg-secondary text-white shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Logo */}
            <div className="flex items-center gap-2 font-bold text-lg flex-shrink-0 group">
              {/* Logo Image - clickable for viewer */}
              <Link 
                href={`/viewer?src=${encodeURIComponent(LOGO_PATH)}&title=SMK+PATRIOT+1+Logo&description=Logo+Sekolah+SMK+PATRIOT+1+BEKASI`}
                className="w-12 h-12 flex-shrink-0 flex items-center justify-center relative hover:opacity-80 transition-opacity cursor-pointer"
              >
                {!logoLoadFailed && (
                  <Image
                    src={LOGO_PATH}
              ...
              {/* School Name - clickable for home */}
